...
def collect_nlu_intent(intent_results):  # pragma: no cover
    ...
    # it could be interesting to include entity-errors later
    # therefore we start with a "intent_errors" key
    intent_values = [{"text": r.message,"intent": r.target,"prediction": r.prediction,"confidence": r.confidence} for r in intent_results]
    csv.register_dialect('myDialect',quoting=csv.QUOTE_ALL,skipinitialspace=True)
    csvfile = csv.writer(open("intent results.csv", "w"), dialect='myDialect')
    ...
    if intent_values:
    	return {'intent_values': intent_values}
    else:
        logger.info("No prediction were found. Empty file")
        return None
...
def evaluate_intents(intent_results,
    ...
    # log and save all intent samples to file for debugging
    results = collect_nlu_intent(intent_results)

    if results:
        save_nlu_errors(results, "intent.json")
...
def get_entity_predictions(interpreter, test_data):  # pragma: no cover
    ...
    EvaluateEntitiesPrediction(interpreter, test_data)
...
def EvaluateEntitiesPrediction(interpreter, test_data):
    ...
    with open("bavage.csv","w",newline='') as filee :
        ...
        with(open('enity_error_script2.csv', 'w')) as file :   
            csv.register_dialect('myDialect',quoting=csv.QUOTE_ALL,skipinitialspace=True)
            ...
            for sentence in test_data.training_examples:
                ...
                row=[]
                row.append(sentence.text.replace(",","").replace("- ","").replace("-","").replace("@","").replace("=","").replace(";","").replace(".","").replace(":",""))
                ...
                for entity in header[2:] :
                    if "Target" in entity:
                        ...
                        # Find Wrong predictions and store them into a list of dict to keep track of the entity predicted
                        if check_result(b,a) == 'Wrongly_Predicted':
                            list_wrong_predictions.append({"target":a,"Wrong prediction":b,"entity": entity})    
                if debug:
                    print("this is the list of wrong prediction : {}\n".format(list_wrong_predictions))
                    print("this is the list of targets : {}\n".format(list_targets))
...
def get_target(sample,entity):
    ...
    if "entities" in sample.data.keys():
        ...
        if entity in dict_target_trained.keys():
            return dict_target_trained[entity].replace(" - ","-").replace("' ","'").replace("@","").replace(" / ","/").replace(".","").replace("☐","☐ ").replace(" ( ","(").replace(" ) ",")")
...
def get_prediction(sample,entity):
    ...
    dict_entities_extracted = {x['entity']:x['value'] for x in sample}  #Create dictionnary of entities extracted with their respective values
    if entity in dict_entities_extracted.keys() :
        ...
        return dict_entities_extracted[entity].replace(" - ","-").replace("' ","'").replace("@","").replace(" / ","/").replace(".","").replace("☐","☐ ").replace(" ( ","(").replace(" ) ",")")
    else :
        return 'vide' 
    
    
def check_bavage(list_wrong,list_targ,sentence):
    
    list_bavage=[]
    list_t= [y['target'] for y in list_targ]
    listp =[x['Wrong prediction'] for x in list_wrong]
    list_bavage.append(sentence.text.replace(",","").replace("- ","").replace("-","").replace("@","").replace("=","").replace(";","").replace(".","").replace(":",""))

    
    #print("list of targets : {}\n ".format(list_t))
    #print("list of wrong_predictions : {}\n ".format(listp))
    for y in list_targ:
    #    list_t.append(y['target'])
        for x in list_wrong:
        
            
            #1er cas de figure : La prédiction est en fait une target d'une autre entité
            if x['Wrong prediction']==y['target']:   
                list_bavage.append(x["Wrong prediction"])
                list_bavage.append(x['entity'])
                list_bavage.append(y['entity'])
            
    #2eme cas de figure : La prédiction n'existe dans aucune autre entité
    
    for l in list_wrong:
        if  l['Wrong prediction'] not in list_t :
            list_bavage.append(x["Wrong prediction"])
            list_bavage.append(x['entity'])
            list_bavage.append(y["target"])
    
    return list_bavage
          
                                       
        
def getListOfEntitiesFromModel(file):
